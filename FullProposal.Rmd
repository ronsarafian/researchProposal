...
- Epidemiologists perspective: prefer high varianced PM measurments, to be aqurate in the village
- measuring prediction errors with the loss - too optimistic (w.r.t epidemiological results)
- loss function - difference between estimation and validation
...
- stationarity - these CV approaches are all suitable only for cases where observation form a general stationary sequence, hence without additional adjustmant, is not applicable for process whose joint probability distribution change when shifted in time or in spatial domain.
evidences for stationary process: 
evidences for unstationary process: increase in number of dust storms: [Krasnov et al, 2016][]; 
still not covered:
...
We point that any parameterized covariance matrix $\Sigma(\theta)$ can be considered as a compromise between two possibilities: The first, is the variance-scalled identity matrix: 

$$\text{Var}(\varepsilon) = \sigma^2 \begin{bmatrix} 1      & 0     & \dots & 0 \\
                                                     0      & 1     &       &   \\
                                                     \vdots &       & \ddots&   \\
                                                     0      &       & \dots & 1 \end{bmatrix},$$

somtimes called *spherical error variance* matrix [Hayashi, 2000][econometrics]. This structure assumes *homoscedasticity* and no autocorrelation, and requires estimation of only one parameter. The second, is the most general model, called the *unstructured covariance matrix* and specifies no patterns:
...
where $\tau^2 = \text{Var}(\varepsilon_{ij}) = \frac{\sigma_{\delta}^2}{1-\rho^2}$. Note that this $AR(1)$ covariance model requires the estimation of 2 parameters, and in general $p+1$ paramrters are required to specify an $AR(p)$ covariance model. 


#### Spatial perspective: 

There are two main approaches discussed in the literature regarding the covariance model in SAC data, each implies a different assumption about the spatial pattern. The *weight matrix* ($W$) approach is particularly common in econometric studies suggests to model the errors generating process (first introduced by [Ord, 1975][estimatio methods...]. This can be described as follows: Consider the model in $(*)$, but now with different assumption regarding to $\varepsilon$: 

$$\varepsilon_i =  \lambda \sum_{k=1}^S w_{ik}\varepsilon_k + \nu_i$$

where $w_{ik}$ is the $i-k$th element in the weight matrix $W$ and $\sum_{k=1}^S w_{ik}\varepsilon_k$ is called the *spatial lag*, since it represent a linear combination of values of the error terms $\varepsilon$ constructed from regions (spatial units) that neighbor $\varepsilon_i$. In matrix notation the $S \times 1$ vector $\varepsilon$ can be represented as:

$$ \varepsilon = (I-\lambda W)^{-1}\nu.$$
Note that $E(\varepsilon) = 0$ and its variance is:

$$ \text{Var}(\varepsilon) = \sigma_{\nu}^2(I-\lambda W)^{-1}(I-\lambda W')^{-1}$$

Many alternatives for choosing the components $w_{ik}$ of $W$ exist, the most common in the literature are:

*k-nearest neighbors*: $$ w_{ik} = \begin{cases} 1 & ,u \in N_k(v) \\ 0 & , otherwise \end{cases} $$

*Radial Distance*: $$ w_{ik} = \begin{cases} 1 & , 0 \leq d_{ik} \leq L \\ 0 & , otherwise \end{cases} $$

*Power Distance*: $$ w_{ik} = \frac{1}{d_{ik}^{\alpha} } $$

how to estimate the parametes

Anoter approach to model the covariance is to specifiy directly the correlation matrix components $K$, assuming some functional form.
...
- Variogram, covariogram and correlogram


#### Spatio-temporal perspective:

Kronecker product of a spatial and a temporal matrix

[Huizenga et al, 2002][Spatiotemporal EEG/MEG Source Analysis Based on a Parametric Noise Covariance Model]

[Naik and Rao, 2001][Analysis of multivariate repeated measures data with a Kronecker product structured covariance matrix] 

[Mardia and Goodall, 1993][Spatial–temporal analysis of multivariate environmental monitoring data]

[Katanoda et al, 2001][A Spatio-temporal Regression Model for the Analysis of Functional MRI Data]

- isotropy vs. unisotropy: follow [Ripley, 2005][spatial statistics]

**Estimation of the covariance model**

Parameters can be estimated jointly with regression parameters using ML - **discuss it** ([Elhorst, 2012][SE]; [Banerjee et. al., 2014][book])

Another estimation approcah is to employ some sort of *feasible generalized least squares* (FGLS): The FGLS is a common method mainly in the econometric field, in which great emphasis is put on data structure. The FGLS modeling proceeds in two stages: (1) the model is estimated ignoring the dependence structure of the data (e.g. with OLS), and the residuals are then used to construct the error covariance matrix estimator $\hat{\Sigma(\theta)}$. (2) The GLS estimation is performed using the first stage estimated $\hat{\Sigma(\theta)}$. 

- FGLS adjusted for spatio-temporal dependence - several stages... **discuss, some ref.**
- discussion: covariance matrix: simultaneous or preset estimation
...
This log likelihood function involves matrix inverse and determinante which might be a difficult task if the matrix dimension is high. However, some dimension reduction formulation can be employed in order to make calculation easier [Demidenko, 2005][mixed models book]
...
- [Pace, 1997][] has suggested the use of sparse matrix techniques to facilitate the use of large samples (computational burden).
...
[Kathryn, Wang, Armstrong][Parallelizing MCMC for Bayesian spatiotemporal geostatistical models]
