...
Note that GLS can be considered as an estimation method that de-correlate the scale of the OLS errors. This means that as long as we reasonably estimated $\Sigma$, strongly dependent observations, wich usually have highly correlated errors, will have less impact on the values of the estimators than independent observations.
...
We point that any parameterized covariance matrix $\Sigma(\theta)$ can be considered as a compromise between two possibilities: The first, is the variance-scalled unit matrix: $\sigma^2 I_N$, somtimes called *spherical error variance* matrix [Hayashi, 2000][econometrics]. This structure assumes *homoscedasticity* and no autocorrelation, and requires estimation of only one parameter. The second, is the most general model, called the *unstructured covariance matrix* and specifies no patterns:
...
Back to the simple linear mixed effect model. A typical assumption is that:

$$ \varepsilon_i \sim \mathcal{N}(0,\sigma^2I) \qquad b_i \sim \mathcal{N}(0,\sigma^2D)$$

and $I=I_{s_j}$.

The multivariate normal distribution of $y_i$ can then be writen as:

$$ y_i \sim \mathcal{N}(X_i\beta, \sigma^2(I+Z_iDZ_i')),$$

and the log likeliyhood function for the linear mixed model is given by:

$$ l(\beta,\sigma^2, D) = 
        -\frac{N_T}{2}\ln{2\pi} 
        -\frac{1}{2} \{N_T \ln{\sigma^2} + 
        \sum_{i=1}^N[\ln{|I+Z_iDZ_i'| \\
                + \sigma^2(y_i-X_i\beta)'(I+Z_iDZ_i')^{-1}(y_i-X_i\beta)}]
        \}$$
...
GLS estimation is essentially applying OLS to the transformed data. To see this, consider $\Sigma$'s Cholesky's decomposition: $\Sigma = L \Lambda L'$ where $L$ is a unitriangular matrix and $\Lambda$ is a diagonal matrix. Easy to prove that:
...
$$ (**) \qquad y^* = X^* \beta + \varepsilon^*, $$
where, $\tilde{y} = Py$, $\tilde{X} = PX$ and $\tilde{\varepsilon} = P\varepsilon$. Note that $E(\tilde{\varepsilon}) = 0$ and $\text{Var}(\tilde{\varepsilon}) = E(P\varepsilon \varepsilon'P') = \sigma^2 P\Sigma P' = \sigma^2I$, Hence $\beta$ can be estimated using $(**)$ and the *ordinary least squares* (OLS):
...
Pen.LME: Howard D Bondell, Arun Krishna, and Sujit K Ghosh. Joint variable selection for fixed and random eects in linear mixed-eects models. Biometrics, 66(4):1069-1077, 2010.

GLMMLASSO: Jurg Schelldorfer, Peter Buhlmann, Sara van de Geer. Estimation for high-dimensional linear mixed-eects models using L1- penalization. Scandinavian Journal of Statistics, 38(2):197-214, 2011

#### Regularization in GLS

As described, **GLS has the effect of whitening the data**////... easy to regularized

In the standard least squares estimation with regularization, $\hat{\beta}_{LS}$ is the minimizer of: 

$$ (y-X\beta)'(y-X\beta)+\lambda g(\beta) $$ 

where $g(\beta)$ is some penalization on model complexity.

Assume $V^{-1}$ is nkown and the Cholesky decomposition is $V^{-1} = LL'$

Then regularized GLS (RGLS) estimates with can have the form:

$$ \hat{\beta}_{RGLS} =\underset{\beta}{\arg\min} \{ (y-X\beta)' V^{-1} (y-X\beta) + \lambda g(\beta) \} $$


$$ = \underset{\beta}{\arg\min}(\textbf{y} - \textbf{X} \beta)' (\textbf{y} - \textbf{X} \beta) + \lambda g(\beta) \} $$
where $\textbf{y} = Ly$ and $\textbf{X} = LX$
